...
class StorageEditScreen extends StatefulWidget {
  ...
  @override State<StorageEditScreen> createState() => _StorageEditScreenState();
}
...
class _StorageEditScreenState extends State<StorageEditScreen> {
  ...
  Future<void> openTier1Picker() async {
    final r = await showDialog<String>(context: context, builder: (c) => _TierSheet(title:'Location (General)', existing: _repo.storageTier1List));
    if (r != null) setState(()=> tier1Controller.text = r);
  }
  Future<void> openTier2Picker() async {
    if (tier1Controller.text.trim().isEmpty) return;
    final r = await showDialog<String>(context: context, builder: (c) => _TierSheet(title:'Location (Specific)', existing: _repo.storageTier2For(tier1Controller.text.trim()), allowBlank:true));
  ...
  Future<void> handleDelete() async {
    final ok = await showDialog<bool>(context: context, builder: (c)=> AlertDialog(title: Text('Delete?'), content: Text('Delete this storage item?'), actions:[TextButton(onPressed: ()=>Navigator.pop(c,false), child: Text('Cancel')), ElevatedButton(onPressed: ()=>Navigator.pop(c,true), child: Text('Delete'))]));
    if (ok == true) { await _repo.deleteStorageItem(widget.itemIndex); if (mounted) Navigator.pop(context); }
  }

  @override Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Edit Storage'), actions:[IconButton(icon: Icon(Icons.delete), onPressed: handleDelete)]),
